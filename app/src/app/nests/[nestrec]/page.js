...
export default function NestDetailPage({ params }) {
  ...
  async function loadNest() {
    try {
      ...
      if (!n.male_id && !n.female_id && n.territory) {
        ...
        if (res.male) setMale({ ...res.male, suggested: true })
        if (res.female) setFemale({ ...res.female, suggested: true })
      }
  ...
  function getWarnings() {
    ...
    if (!nest.male_id) {
      ...
      w.push({ t: 'action', m: 'Male parent is unbanded — band via Birds tab', link: '/birds' })
    }
    if (!nest.female_id) {
      ...
      w.push({ t: 'action', m: 'Female parent is unbanded — band via Birds tab', link: '/birds' })
    }
    return w
  }
          ...
          <>
            ...
            <div className="bg-gray-50 border-x border-gray-300 p-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <label className="block text-[11px] text-gray-600 mb-0.5">Stage found</label>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <div className="text-[10px] text-gray-400">Auto-calculated from chick age visits</div>
                  </div>
                  <div>
                    <label className="block text-[11px] text-gray-600 mb-0.5">
                      DFE <span className="text-gray-400">(date of first egg)</span>
                    </label>
                    ...
                    <div className="text-[10px] text-gray-400">Auto: hatch - 13 - (eggs-1)</div>
                  </div>
                  <div>
                    <label className="block text-[11px] text-gray-600 mb-0.5">
                      Corrected DFE
                    </label>
                    <input type="number" value={card.corr_dfe}
                      onChange={e => setCard({...card, corr_dfe: e.target.value})}
                      placeholder="—"
                      className="w-full border rounded px-2 py-1.5 text-sm font-mono bg-white" />
                    <div className="text-[10px] text-gray-400">From 2014 data review; used for analysis if present</div>
                  </div>
                </div>
              </div>
            ...
            {/* Save / Cancel */}
            <div className="bg-white rounded-b-lg border p-4 flex gap-2">
